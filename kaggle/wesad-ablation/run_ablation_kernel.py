"""
Kaggle script kernel: KD hyperparameter ablation (4x4 grid) under nested LOSO.
...
teacher checkpoints found there into this run's MODELS_DIR before the sweep.

Not meant to be edited by hand for each run -- kaggle_runner.py rewrites
PINNED_SHA, SMOKE, SWEEP and MODEL below before every `kaggle kernels push`.
"""
...
KERNEL_SLUG = "euphora/wesad-ablation"
...
def main() -> None:
    ...
    cmd = [sys.executable, 'run_ablation.py', '--model', MODEL]
    if SMOKE:
        cmd.append('--smoke')
    else:
        cmd += ['--sweep', SWEEP]
    ...
    assert result.returncode == 0, f'run_ablation.py failed with exit code {result.returncode}'
